...
public:
    ...
    void updatePositionUsingGraph(float deltaTime)
    {
        ...
        if (currentNodeIndex < path.size() - 1) {
            ...
            for (const auto& polygon : map->collisionList) {
                if (willCollide(mapToScreenCoordinates(playerPosition), nextNodePosition, polygon)) {
            ...
            else if (playerPosition.distance(map->graph->vertices[path[currentNodeIndex]].getPosition()) < 0.02) {
            ...
            bool collisionDetected = false;
            for (const auto& polygon : map->collisionList) {
                if (willCollide(mapToScreenCoordinates(playerPosition), targetPosition, polygon)) {
                    collisionDetected = true;
                    break;
                }
            }
